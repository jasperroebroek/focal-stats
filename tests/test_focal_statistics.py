...
def test_focal_stats_values():
    ...
    assert np.allclose(focal_stats.focal_min(a)[2, 2], a.min())
    assert np.allclose(focal_stats.focal_max(a)[2, 2], a.max())
    assert np.allclose(focal_stats.focal_std(a)[2, 2], a.std())
    assert np.allclose(focal_stats.focal_std(a, std_df=1)[2, 2], a.std(ddof=1))

    # Values when reducing
    assert np.allclose(focal_stats.focal_mean(a, reduce=True)[0, 0], a.mean())
    assert np.allclose(focal_stats.focal_sum(a, reduce=True)[0, 0], a.sum())
    assert np.allclose(focal_stats.focal_min(a, reduce=True)[0, 0], a.min())
    assert np.allclose(focal_stats.focal_max(a, reduce=True)[0, 0], a.max())
    assert np.allclose(focal_stats.focal_std(a, reduce=True)[0, 0], a.std())
    assert np.allclose(focal_stats.focal_std(a, std_df=1, reduce=True)[0, 0], a.std(ddof=1))
    ...
    assert scipy.stats.mode(a.flatten()).mode[0] == \
           focal_stats.focal_majority(a, window_size=5, majority_mode='ascending')[2, 2]
    # Values when not reducing
    assert scipy.stats.mode(a.flatten()).mode[0] == \
           focal_stats.focal_majority(a, window_size=5, reduce=True, majority_mode='ascending')[0, 0]
...
def test_focal_stats_values_mask(fs, np_fs):
    ...
    assert np.allclose(fs(a, mask=mask)[2:-2, 3:-3], np_fs(focal_stats.rolling_window(a, mask=mask), axis=-1))
...
def test_focal_stats_nan_behaviour_majority():
    ...
    a[1, 1] = np.nan
    assert focal_stats.focal_majority(a)[2, 2] == 1
    assert not np.isnan(focal_stats.focal_majority(a, fraction_accepted=0)[2, 2])
    assert np.isnan(focal_stats.focal_majority(a, fraction_accepted=1)[2, 2])
