...
def test_focal_stats_values():
    ...
    assert np.allclose(focal_stats.focal_std(a, window=5, std_df=1)[2, 2], a.std(ddof=1))
    ...
    assert np.allclose(focal_stats.focal_std(a, window=5, std_df=1, reduce=True),
                       focal_stats.rolling_window(a, window=5, reduce=True, flatten=True).std(axis=-1, ddof=1))
    ...
    # Values when reducing
    assert focal_stats.focal_majority(a, window=5, majority_mode='ascending')[2, 2] == mode
    # Values when not reducing
    assert focal_stats.focal_majority(a, window=5, reduce=True, majority_mode='ascending')[0, 0] == mode

    # Same number of observations in several classes lead to NaN in majority_mode='nan'
    a = np.arange(100).reshape(10, 10)
    assert np.isnan(focal_stats.focal_majority(a, window=10, reduce=True, majority_mode='nan'))

    # Same number of observations in several classes lead to lowest number in majority_mode='ascending'
    assert focal_stats.focal_majority(a, window=10, reduce=True, majority_mode='ascending') == 0

    # Same number of observations in several classes lead to highest number in majority_mode='descending'
    assert focal_stats.focal_majority(a, window=10, reduce=True, majority_mode='descending') == 99
...
def test_focal_stats_nan_behaviour_majority():
    ...
    assert not np.isnan(focal_stats.focal_majority(a, window=5, fraction_accepted=0)[2, 2])
